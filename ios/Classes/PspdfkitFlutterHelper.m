...
//  INTERNATIONAL COPYRIGHT LAW AND MAY NOT BE RESOLD OR REDISTRIBUTED. USAGE IS
//  BOUND TO THE PSPDFKIT LICENSE AGREEMENT. UNAUTHORIZED REPRODUCTION OR
//  DISTRIBUTION IS SUBJECT TO CIVIL AND CRIMINAL PENALTIES. This notice may not
...
+ (void)processMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"save" isEqualToString:call.method]) {
    ...
    PSPDFDataContainerProvider *jsonContainer =
        [[PSPDFDataContainerProvider alloc]
            initWithData:[annotationsJson
                             dataUsingEncoding:NSUTF8StringEncoding]];
    NSError *error;
    BOOL success = [document
        applyInstantJSONFromDataProvider:jsonContainer
    ...
    NSError *error;
    NSData *data = [document
        generateInstantJSONFromDocumentProvider:document.documentProviders
                                                    .firstObject
                                          error:&error];
    ...
    result([PspdfkitFlutterHelper
        getAnnotationsForPageIndex:pageIndex
                           andType:typeString
                 forViewController:pdfViewController]);
  } else if ([@"getAllUnsavedAnnotations" isEqualToString:call.method]) {
    result([PspdfkitFlutterHelper
        getAllUnsavedAnnotationsForViewController:pdfViewController]);
  } else if ([@"importXfdf" isEqualToString:call.method]) {
    NSString *path = call.arguments[@"xfdfPath"];
    result([PspdfkitFlutterHelper importXFDFFromPath:path
                                   forViewController:pdfViewController]);
  } else if ([@"exportXfdf" isEqualToString:call.method]) {
    NSString *path = call.arguments[@"xfdfPath"];
    result([PspdfkitFlutterHelper exportXFDFToPath:path
                                 forViewController:pdfViewController]);
    ...
    NSURL *processedDocumentURL =
        [PspdfkitFlutterHelper writableFileURLWithPath:outputPath
                                              override:YES
                                          copyIfNeeded:NO];
    NSDictionary *options = call.arguments[@"options"];
    [PspdfkitHtmlPdfConvertor
        generateFromHtmlStringWithHtml:html
                         outputFileURL:processedDocumentURL
                     convertionOptions:options
                               results:result];
    ...
    NSDictionary *options = call.arguments[@"options"];
    [PspdfkitHtmlPdfConvertor
        generateFromHtmlURLWithHtmlURL:htmlURL
                         outputFileURL:processedDocumentURL
                     convertionOptions:options
                               results:result];
  } else if ([@"generatePDF" isEqualToString:call.method]) {
    NSString *outputPath = call.arguments[@"outputFilePath"];
    NSArray<NSDictionary<NSString *, NSObject *> *> *pages =
        call.arguments[@"pages"];
    NSURL *processedDocumentURL =
        [PspdfkitFlutterHelper writableFileURLWithPath:outputPath
                                              override:YES
                                          copyIfNeeded:NO];
    [PspdfkitPdfGenerator generatePdfWithPages:pages
                                     outputUrl:processedDocumentURL
                                       results:result];
    ...
    // if pdfViewController is an instance of InstantDocumentViewController,
    // then we can set the delay
    if ([pdfViewController
            isKindOfClass:[InstantDocumentViewController class]]) {
      InstantDocumentViewController *instantDocumentViewController =
          (InstantDocumentViewController *)pdfViewController;
      instantDocumentViewController.documentDescriptor
          .delayForSyncingLocalChanges = [delay doubleValue];
      result(@(YES));
    } else {
      result([FlutterError
          errorWithCode:@"InvalidArgument"
    ...
    if ([pdfViewController
      ...
      InstantDocumentViewController *instantDocumentViewController =
          (InstantDocumentViewController *)pdfViewController;
      instantDocumentViewController.shouldListenForServerChangesWhenVisible =
          listenToServerChanges;
      result(@(YES));
    } else {
      result([FlutterError errorWithCode:@"InvalidArgument"
                                 message:@"listenToServerChanges can only be "
                                         @"set for Instant documents"
                                 details:nil]);
    }
    ...
    if ([pdfViewController
      ...
      result([FlutterError
          errorWithCode:@"InvalidArgument"
                message:
                    @"syncAnnotations can only be called on Instant document"
                details:nil]);
    }
    ...
    PSPDFMeasurementScale *measurementScale =
        [PspdfkitMeasurementConvertor convertScaleWithMeasurement:scale];

    if (!measurementScale) {
      result([FlutterError errorWithCode:@""
                                 message:@"Measurement scale is invalid."
                                 details:nil]);
      return;
    }
    ...
    PSPDFMeasurementPrecision measurementPrecision =
        [PspdfkitMeasurementConvertor convertPrecisionWithPrecision:precision];

    if (!measurementPrecision) {
      result([FlutterError errorWithCode:@""
                                 message:@"Measurement precision is invalid."
                                 details:nil]);
      return;
    }
  ...
  } else if ([@"setAnnotationPresetConfigurations"
                 isEqualToString:call.method]) {
    [AnnotationsPresetConfigurations
        setConfigurationsWithAnnotationPreset:
            call.arguments[@"annotationConfigurations"]];
    result(nil);
  } else {
...
+ (NSURL *)writableFileURLWithPath:(NSString *)path
  ...
  if (path.absolutePath) {
    ...
    NSString *docsFolder = NSSearchPathForDirectoriesInDomains(
                               NSDocumentDirectory, NSUserDomainMask, YES)
                               .firstObject;
    writableFileURL = [NSURL
        fileURLWithPath:[docsFolder stringByAppendingPathComponent:path]];
  }
  ...
  // If we don't have a writable file already, we move the provided file to the
  // ~/Documents folder.
  if (![fileManager fileExistsAtPath:(NSString *)writableFileURL.path]) {
    ...
    if (![fileManager
                  createDirectoryAtPath:writableFileURL.path
                                            .stringByDeletingLastPathComponent
            withIntermediateDirectories:YES
                             attributes:nil
                                  error:&createFolderError]) {
      NSLog(@"Failed to create directory: %@",
            createFolderError.localizedDescription);
      return nil;
    }
    ...
    NSError *copyError;
    if (copyIfNeeded &&
        [fileManager fileExistsAtPath:(NSString *)fileURL.path]) {
      if (![fileManager copyItemAtURL:fileURL
                                toURL:writableFileURL
                                error:&copyError]) {
...
+ (void)unlockWithPasswordIfNeeded:(PSPDFDocument *)document
...
+ (void)setToolbarTitle:(NSString *)toolbarTitle
      forViewController:(PSPDFViewController *)pdfViewController {
  // Early return if the toolbar title is not explicitly set in the
  ...
  pdfViewController.title =
      (id)toolbarTitle == NSNull.null ? nil : toolbarTitle;
}
...
+ (void)setLeftBarButtonItems:(nullable NSArray<NSString *> *)items
  ...
  for (NSString *barButtonItemString in items) {
    UIBarButtonItem *barButtonItem =
        [self barButtonItemFromString:barButtonItemString
                    forViewController:pdfViewController];
    if (barButtonItem && ![pdfViewController.navigationItem.rightBarButtonItems
                             containsObject:barButtonItem]) {
      [leftItems addObject:barButtonItem];
    }
  }

  [pdfViewController.navigationItem setLeftBarButtonItems:[leftItems copy]
                                                 animated:NO];
}
  ...
  for (NSString *barButtonItemString in items) {
    UIBarButtonItem *barButtonItem =
        [PspdfkitFlutterHelper barButtonItemFromString:barButtonItemString
                                     forViewController:pdfViewController];
    if (barButtonItem && ![pdfViewController.navigationItem.leftBarButtonItems
                             containsObject:barButtonItem]) {
      [rightItems addObject:barButtonItem];
    }
  }

  [pdfViewController.navigationItem setRightBarButtonItems:[rightItems copy]
                                                  animated:NO];
}

+ (void)setDocumentInfoOptions:(nullable NSArray<NSString *> *)options
            forViewController:(PSPDFViewController *)pdfViewController {
  if ((id)options == NSNull.null || !options || options.count == 0) {
    return;
  }
  NSMutableArray *infoOptions = [NSMutableArray array];
  for (NSString *documentInfoOptionString in options) {
      PSPDFDocumentInfoOption documentInfoOption =
        [PspdfkitFlutterHelper documentInfoOptionFromString:documentInfoOptionString];
    if (documentInfoOption) {
      [infoOptions addObject:documentInfoOption];
    }
  }

  [pdfViewController.documentInfoCoordinator setAvailableControllerOptions:[infoOptions copy]];
}
...
+ (PSPDFDocumentInfoOption)
    documentInfoOptionFromString:(NSString *)documentInfoOption {
  if ([documentInfoOption isEqualToString:@"outline"]) {
    return PSPDFDocumentInfoOptionOutline;
  } else if ([documentInfoOption isEqualToString:@"annotations"]) {
    return PSPDFDocumentInfoOptionAnnotations;
  } else if ([documentInfoOption isEqualToString:@"bookmarks"]) {
    return PSPDFDocumentInfoOptionBookmarks;
  } else if ([documentInfoOption isEqualToString:@"embeddedFiles"]) {
    return PSPDFDocumentInfoOptionEmbeddedFiles;
  } else if ([documentInfoOption isEqualToString:@"documentInfo"]) {
    return PSPDFDocumentInfoOptionDocumentInfo;
  } else if ([documentInfoOption isEqualToString:@"security"]) {
    return PSPDFDocumentInfoOptionSecurity;
  } else {
    return nil;
  }
}
...
+ (id)setFormFieldValue:(NSString *)value
  ...
  for (PSPDFFormElement *formElement in document.formParser.forms) {
    if ([formElement.fullyQualifiedFieldName
            isEqualToString:fullyQualifiedName]) {
  ...
  if (!success) {
    FlutterError *error = [FlutterError
        errorWithCode:@""
              message:[NSString stringWithFormat:@"Error while searching for a "
...
+ (id)getFormFieldValueForFieldWithFullyQualifiedName:
          (NSString *)fullyQualifiedName
                                    forViewController:(PSPDFViewController *)
                                                          pdfViewController {
  if (fullyQualifiedName == nil || fullyQualifiedName.length == 0) {
    FlutterError *error = [FlutterError
        errorWithCode:@""
...
+ (id)processAnnotationsOfType:(NSString *)type
  ...
  NSURL *processedDocumentURL =
      [PspdfkitFlutterHelper writableFileURLWithPath:destinationPath
                                            override:YES
                                        copyIfNeeded:NO];
  PSPDFAnnotationType annotationType =
      [PspdfkitFlutterConverter annotationTypeFromString:type];
  ...
  PSPDFProcessor *processor =
      [[PSPDFProcessor alloc] initWithConfiguration:configuration
                                    securityOptions:nil];
...
+ (id)addAnnotation:(id)jsonAnnotation
  ...
  if ([jsonAnnotation isKindOfClass:NSString.class]) {
    ...
    data = [NSJSONSerialization dataWithJSONObject:jsonAnnotation
                                           options:0
                                             error:nil];
  }
  ...
  if (data == nil) {
    return [FlutterError errorWithCode:@""
                               message:@"Invalid JSON Annotation."
                               details:nil];
  }

  PSPDFDocumentProvider *documentProvider =
      document.documentProviders.firstObject;
  PSPDFAnnotation *annotation =
      [PSPDFAnnotation annotationFromInstantJSON:data
...
+ (id)removeAnnotation:(id)jsonAnnotation
  ...
  if (annotationUUID.length <= 0) {
    return [FlutterError errorWithCode:@""
                               message:@"Invalid annotation UUID."
                               details:nil];
  }
...
+ (id)getAnnotationsForPageIndex:(PSPDFPageIndex)pageIndex
  ...
  PSPDFAnnotationType type =
      [PspdfkitFlutterConverter annotationTypeFromString:typeString];

  NSArray<PSPDFAnnotation *> *annotations =
      [document annotationsForPageAtIndex:pageIndex type:type];
...
+ (id)getAllUnsavedAnnotationsForViewController:
    (PSPDFViewController *)pdfViewController {
  PSPDFDocument *document = pdfViewController.document;
  if (!document || !document.isValid) {
    return [FlutterError errorWithCode:@""
  ...
  PSPDFDocumentProvider *documentProvider =
      document.documentProviders.firstObject;
  NSData *data =
      [document generateInstantJSONFromDocumentProvider:documentProvider
                                                  error:NULL];
  NSDictionary *annotationsJSON =
...
+ (id)importXFDFFromPath:(NSString *)path
  ...
  if (![NSFileManager.defaultManager
          fileExistsAtPath:(NSString *)fileURL.path]) {
    return [FlutterError errorWithCode:@""
  ...
  PSPDFFileDataProvider *dataProvider =
      [[PSPDFFileDataProvider alloc] initWithFileURL:fileURL];
  PSPDFXFDFParser *parser = [[PSPDFXFDFParser alloc]
...
+ (id)exportXFDFToPath:(NSString *)path
  ...
  for (NSArray *pageAnnotations in
       [document allAnnotationsOfType:PSPDFAnnotationTypeAll]
           .allValues) {
  ...
  NSError *error;
  PSPDFFileDataSink *dataSink =
      [[PSPDFFileDataSink alloc] initWithFileURL:fileURL
